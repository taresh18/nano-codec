...
from utils import (
    save_loss_plot, save_checkpoint, load_checkpoint, setup_exp_dir, save_config,
    Profiler, MetricsTracker, WandbLogger, reconstruct_samples,
)
...
def train(cfg, model, loader, dataset, optimiser, scheduler, criterion, device,
          exp_dir, ckpt_dir, samples_dir, start_epoch, epoch_recon_losses,
          epoch_commit_losses, best_recon_loss, use_wandb=True):

    eval_indices = torch.randperm(len(dataset))[:cfg.get('num_eval_samples', 4)].tolist()
    ...
    profiler = Profiler(enabled=cfg.get('profile', False))
    metrics = MetricsTracker()
    logger = WandbLogger(use_wandb, cfg.get('log_interval', 50))
    ...
    for epoch in range(start_epoch, cfg['num_epochs']):
        ...
        for batch_idx, x in enumerate(pbar):
            profiler.mark('start', batch_idx)

            # forward
            x = x.to(device, non_blocking=True)
            profiler.mark('fwd', batch_idx)

            with torch.amp.autocast(device_type='cuda', enabled=use_amp):
                x_recon, indices, commitment_loss, codebook_loss = model(x)
            profiler.mark('loss', batch_idx)

            # claculate spectral losses in fp32
            x_recon = x_recon.float()[..., :x.shape[-1]]
            recon_loss, recon_details = criterion(x.float(), x_recon)
            raw_total = recon_loss + beta * commitment_loss.float() + codebook_loss.float()
            profiler.mark('bwd', batch_idx)

            # backward
            scaler.scale(raw_total / grad_accum_steps).backward()
            ...
            profiler.mark('end', batch_idx)
            profiler.report(batch_idx)

            # tracking
            metrics.update(recon_details, commitment_loss, codebook_loss)
            pbar.set_postfix(recon=f"{recon_details['recon_total']:.5f}", commit=f"{commitment_loss.item():.5f}", cb=f"{codebook_loss.item():.5f}")
            logger.log_batch(batch_idx, recon_details, commitment_loss, codebook_loss, raw_total)
        ...
        # ── epoch summary ──
        avg_recon, avg_commit, avg_cb = metrics.averages()
        ...
        logger.log_epoch(epoch, avg_recon, avg_commit, avg_cb, scheduler.get_last_lr()[0])
        ...
        if avg_recon < best_recon_loss:
            ...
            reconstruct_samples(model, dataset, eval_indices, device, use_amp, epoch,
                                samples_dir, cfg['sample_rate'], logger)
